use glam::DVec3;
use opencascade::{
    primitives::{IntoShape, Shape},
    workplane::Workplane,
    Error,
};

pub fn shape() -> Result<Shape, Error> {
    let edge_length_1 = 48.0;
    let hole_diamet_1 = 23.0;
    let wall_thickn_1 = 5.0;

    let edge_length_2 = edge_len_from_circum_diam(edge_length_1 - 2.0 * wall_thickn_1);
    let hole_diamet_2 = 11.0;
    let wall_thickn_2 = 4.0;

    let edge_length_3 = edge_len_from_circum_diam(edge_length_2 - 2.0 * wall_thickn_2);

    let c1 = hollow_cube(edge_length_1, hole_diamet_1, wall_thickn_1)?;
    let c2 = hollow_cube(edge_length_2, hole_diamet_2, wall_thickn_2)?;
    let c3 = cube(edge_length_3)?;

    Ok(c1.union(&c2)?.union(&c3)?.into_shape())
}

fn edge_len_from_circum_diam(d: f64) -> f64 {
    d / f64::sqrt(2.0)
}

fn hollow_cube(edge_length: f64, hole_diameter: f64, wall_thickness: f64) -> Result<Shape, Error> {
    let r1 = 0.5 * hole_diameter;
    let h1 = edge_length;
    let r2 = 0.5 * edge_length - wall_thickness;
    let h2 = edge_length - 2.0 * wall_thickness;

    let p1 = rolling_pin(&Workplane::xy(), r1, h1, r2, h2)?;
    let p2 = rolling_pin(&Workplane::yz(), r1, h1, r2, h2)?;
    let p3 = rolling_pin(&Workplane::zx(), r1, h1, r2, h2)?;

    let cutout = p1.union(&p2)?.union(&p3)?.into_shape();

    Ok(cube(edge_length)?.subtract(&cutout)?.into_shape())
}

fn cube(edge_length: f64) -> Result<Shape, Error> {
    let wp = Workplane::xy();
    let wp = wp.translated(DVec3::new(0.0, 0.0, -0.5 * edge_length));
    let wire = wp.rect(edge_length, edge_length);
    Ok(wire.to_face()?.extrude(DVec3::new(0.0, 0.0, edge_length)).into_shape())
}

fn rolling_pin(wp: &Workplane, r1: f64, h1: f64, r2: f64, h2: f64) -> Result<Shape, Error> {
    let c_long = cylinder(wp, r1, h1)?;
    let c_wide = cylinder(wp, r2, h2)?;
    Ok(c_long.union(&c_wide)?.into_shape())
}

fn cylinder(wp: &Workplane, radius: f64, height: f64) -> Result<Shape, Error> {
    let wp = wp.translated(DVec3::new(0.0, 0.0, -0.5 * height));
    let wire = wp.circle(0.0, 0.0, radius);
    Ok(wire.to_face()?.extrude(height * wp.normal()).into_shape())
}
